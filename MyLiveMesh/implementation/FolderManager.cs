...
namespace MyLiveMesh.implementation
{
    ...
    public class FolderManager : IFolderManager
    {
        ...
        public WebResult Create(int userId, string name)
        {
            ...
            string path = System.IO.Path.Combine(HttpContext.Current.Server.MapPath("~"), "upload_files", user.username, name);
        ...
        public WebResult Rename(int userId, string name, string newName)
        {
            ...
            if (user == default(User))
                return new WebResult(WebResult.ErrorCodeList.USER_NOT_FOUND);
            string path = System.IO.Path.Combine(HttpContext.Current.Server.MapPath("~"), "upload_files", user.username, name);
            string newPath = System.IO.Path.Combine(HttpContext.Current.Server.MapPath("~"), "upload_files", user.username, newName);
            if (System.IO.Directory.Exists(path))
            {
        ...
        public WebResult<List<string>> DirList(int userId)
        {
            ...
            string path = System.IO.Path.Combine(HttpContext.Current.Server.MapPath("~"), "upload_files", user.username);
        ...
        public WebResult<List<FileDefinition>> FileList(int userId, string folder)
        {
            ...
            if (System.IO.Directory.Exists(path) == false)
                return new WebResult<List<FileDefinition>>(WebResult.ErrorCodeList.DIRECTORY_NOT_FOUND);
            DirectoryInfo dirInfo = new DirectoryInfo(path);
        ...
        {
            var share = (from s in db.Sharings where s.id == sharingId select s).SingleOrDefault();

            if (share == default(Sharing))
                return new WebResult(WebResult.ErrorCodeList.INFORMATION_REQUIRED);
            db.Sharings.DeleteOnSubmit(share);
            db.SubmitChanges();
            return new WebResult();
        }
    }
